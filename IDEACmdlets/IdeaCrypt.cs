...
namespace IDEACmdlets
{
    ...
    public class IdeaCrypt
    {
        ...
        public static void CryptFile(MemoryStream inStream, String outputFileName, String charKey, bool encrypt)
        {
            FileStream outStream = null;
            try
            {
        ...
        private static void processData(MemoryStream inStream, long inDataLen, FileStream outStream, long outDataLen, BlockStreamCrypter bsc)
        {
            int bufSize = 0x200000;
            byte[] buf = new byte[bufSize];
            long filePos = 0;
        ...
        private static long readDataLength(MemoryStream stream, BlockStreamCrypter bsc)
        {
            byte[] buf = new byte[blockSize];
            int trLen = stream.Read(buf, 0, blockSize);
            if (trLen != blockSize)
